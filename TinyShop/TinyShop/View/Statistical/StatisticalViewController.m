...
-(void)addLineViewbrefixStr:(NSString *)beforeStr sumValue:(NSString *)RTSumValue unit:(NSString *)unit{
    _lineView = [[MZLineView alloc]initWithFrame:CGRectMake(0, 145, self.view.width,self.view.height-144)];
...
-(void)loadDatatype:(NSString *)type queryTime:(NSString *)queryTime{
    ...
    //数据请求成功之后绘制 折线图
    [self addLineViewbrefixStr:@"收入" sumValue:@"实时总收入" unit:@"元" ];
}
...
-(void)fiveBtnClicked:(UIButton *)btn{
    ...
    switch (btn.tag) {
        case 1000:
            ...
            [self addLineViewbrefixStr:@"桌数" sumValue:@"总桌数" unit:@"桌" ];
            break;
            case 1002:
            [self.lineView removeFromSuperview];
            [self loadDatatype:@"order_num" queryTime:@"day"];
            [self addLineViewbrefixStr:@"人数" sumValue:@"总人数" unit:@"人" ];
            break;
            case 1003:
            [self.lineView removeFromSuperview];
            [self addLineViewbrefixStr:@"人均" sumValue:@"总人均" unit:@"元" ];
            break;
        default:
            [self.lineView removeFromSuperview];
            [self addLineViewbrefixStr:@"餐时" sumValue:@"餐时" unit:@"时" ];
            break;
    }
}
-(void)addPopView{
